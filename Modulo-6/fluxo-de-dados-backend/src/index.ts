...
const Errors: { [chave: string]: { status: number, message: string } } = {
  ...
  BAD_REQUEST: { status: 400, message: "Valor inválido" },
}
  ...
  try {
    ...
    switch (error.message) {
      ...
      case Errors.BAD_REQUEST.message:
        response.status(Errors.BAD_REQUEST.status).send(Errors.BAD_REQUEST.message);
        break;
      default:
...
// Exercícios 5 e 8  // o exercicio 8 ainda falta 1 verificação.
...
app.put("/users/edit", (request: Request, response: Response) => {
  ...
  try {
    ...
    if (!paramId || !paramPrice) { throw new Error(Errors.MISSING_PARAMETERS.message) }
    if (typeof paramPrice !== "number" || paramPrice < 0) { throw new Error(Errors.BAD_REQUEST.message) }

    let produtosAtualizados = productsList.map((produto) => {
      if (produto.id === paramId) { produto.price = paramPrice }
      return produto
    })
